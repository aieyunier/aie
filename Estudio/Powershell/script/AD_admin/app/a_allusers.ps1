...
$tablaUsuarios = $tablaUsuarios | Sort-Object -Property LastLogonDate -Descending
# Mostrar la lista en formato de tabla
$tablaUsuarios | Format-Table -AutoSize LastLogonDate, PasswordLastSet, Name, Mail
$text =  $tablaUsuarios | Format-Table -AutoSize LastLogonDate, PasswordLastSet, Name, Mail
write-host "****************************************************************************************"
# Exportar el resultado a un archivo de texto
$fechaActual = Get-Date
$reportFileName = $fechaActual.ToString("yyyyMMdd_HHmmss") + "_reporte_allUsers.txt"
$text | Out-File -FilePath ".\reportes\$reportFileName"
