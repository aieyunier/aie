...
clear-host
Write-host "#################################################################"
Write-host "########## REPORTE DE USUARIOS BLOQUEADOS DEL AD ################"
Write-host "#################################################################"
...
$csvPath = "C:\Export\lockedUser.csv"
#################################################################################################################################
...
$user = get-aduser -Credential $credenciales -filter * -properties Name, PasswordNeverExpires, mail, PasswordExpired, PasswordLastSet, PasswordExpired, LastLogonDate, Enabled, BadPasswordTime
write-host "Total de usuarios usuarios:................ " $user.Length
# obteniendo los usuarios con   uw se encuentran deshabilitados
$lockedUser= get-aduser -Credential $credenciales -filter * -properties Name, PasswordNeverExpires, mail, PasswordExpired, PasswordLastSet, PasswordExpired, LastLogonDate, Enabled, BadPasswordTime | Where-Object { $_.LockedOut -eq $True }
write-host "Usuarios Bloqueados:................. " $disabledUser.Length
write-host "#################################################################"
write-host "Lista de usuarios bloqueados:"
write-host "Nomre_Usuario ____________ Fecha Bloqueo"


foreach ($user in $disabledUser) {	
	$Name = (Get-ADUser $user | ForEach-Object { $_.Name})	
	$fechaBloqueo = [DateTime]::FromFileTime($lockedUser.BadPasswordTime)
	write-host $user.Name `t $user.LastLogonDate
	write-host $user.Name ____________ $fechaBloqueo
}										
