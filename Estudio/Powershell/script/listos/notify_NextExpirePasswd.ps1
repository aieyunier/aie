...
Clear-Host
Write-host "#################################################################"
Write-host "########## NOTIFICACION A USUARIOS PROXIMOS A EXPIRAR ###########"
Write-host "#################################################################"
...
# Install-Module -Name ActiveDirectory.Toolbox  (este es el que funciona a veces)
...
# copiandolo para la carpeta de modulos de powershell: C:\Windows\system32\WindowsPowerShell\v1.0\Modules
...
$users = get-aduser -Credential $credenciales -filter * -properties Name, PasswordNeverExpires, mail, PasswordExpired, PasswordLastSet |Where-Object {$_.Enabled -eq "True"} | Where-Object { $_.PasswordNeverExpires -eq $false } | Where-Object { $_.passwordexpired -eq $false }
...
foreach ($user in $users) {	# se optiene la prima cadena de usuario (DN completo del usuario)
	$Name = (Get-ADUser $user | ForEach-Object { $_.Name})		 # Se extrae el nombre de usuario de la cadena   		
