...
$search = read-host "Escriba el patron de busqueda para listar los usuarios
Ejmplos:
* 		todos los usuarios
*pepe* 	usuarios que en el nombre contengan pepe
"

# leyendo la base de datos de usuarios del servidor LDAP
write-host "Obteniendo el listado de usuarios de directorio..."
$allUsers = get-aduser -Credential $credenciales -filter * -properties Name, PasswordNeverExpires, mail, PasswordExpired, PasswordLastSet, PasswordExpired, LastLogonDate, Enabled | Where-Object { $_.Name -like $search }
...
write-host "********************************************************************************"
write-host "Name				Mail		PasswordLastSet			LastLogonDtate"
foreach ($user in $allUsers) {	
	$Name = (Get-ADUser $user | ForEach-Object { $_.Name})	
	#write-host $user.Name `t $user.mail `t $user.PasswordLastSet `t $user.LastLogonDate
	write-host $user.Name `t $user.mail `t $user.PasswordLastSet `t $user.LastLogonDate
}		
